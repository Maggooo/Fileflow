...
Ruleaza: python3 fileflow.py
"""

import os, shutil, hashlib, json, threading, webbrowser, secrets
...
def obtine_data_creare(cale):
    # 1. EXIF DateTimeOriginal (cel mai precis pentru poze)
    d = data_exif(cale)
    if d:
        return d
    # 2. Fallback pe data fisierului
    s = os.stat(cale)
    if hasattr(s, "st_birthtime"):
        return datetime.fromtimestamp(s.st_birthtime)
...
def organizeaza(cfg, progress_cb):
    ...
    mod_organizare = cfg.get("mod_organizare", "ext")
    ...
    def inregistreaza(entry, status, stat_key, **kw):
        entry.update(status=status, **kw)
        log.append(entry)
    ...
    for idx, cale in enumerate(fisiere):
        ...
        ext_grup  = ext_bruta or "fara_extensie"
        try:    marime_kb = os.path.getsize(cale) / 1024
        except Exception: marime_kb = 0
        ...
        # 1. Filtrare extensie
        if ext_inc and ext_grup not in ext_inc:
            inregistreaza(entry, "omis-ext", "omis_ext", motiv=f"extensie '{ext_grup}' neinclusă")
            continue
        if ext_grup in ext_exc:
            inregistreaza(entry, "omis-ext", "omis_ext", motiv=f"extensie '{ext_grup}' exclusă")
            continue
        ...
        if sterge_mici and marime_kb < limita_kb:
            if not dry_run:
                ...
                try:
                    ...
                    inregistreaza(entry, "eroare", "erori", motiv=str(e))
                    continue
            else:
                status = "dry-run"
            inregistreaza(entry, status, "sters", motiv=f"< {limita_kb} KB")
            continue
        ...
        if omit_dup:
            try:
                ...
                if h in hashuri:
                    ...
                    inregistreaza(entry, "duplicat", "duplicate", motiv=f"egal cu '{orig}'")
                    continue
                hashuri[h] = cale
            except Exception:
                h = None
        ...
        # 4. Calcul destinatie
        if mod_organizare == "structura":
            rel_dir  = os.path.dirname(os.path.relpath(cale, src))
            folder_d = os.path.join(dest, rel_dir) if rel_dir else dest
        else:
            data     = obtine_data_creare(cale)
            folder_d = os.path.join(dest, ext_grup, str(data.year), data.strftime("%m-%B"))
        ...
        try:
            if not dry_run:
                ...
                if omit_dup and h is not None: hashuri[h] = cale_dest
            status = "dry-run" if dry_run else actiune.lower()
            inregistreaza(entry, status, "procesat", dest=dest_rel)
        except Exception as e:
            inregistreaza(entry, "eroare", "erori", motiv=str(e))
...
<div class="wrap">
  ...
  <div id="config-section">
    ...
    <div class="card">
      ...
      <button type="button" class="btn-back" id="btn-scan" style="margin-top:0;margin-bottom:14px" onclick="scaneazaExtensii()">Scanează sursa</button>
      <div id="ext-chips" style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px"></div>
      <div class="grid2">
        <div class="field">
    ...
    <div class="card">
      ...
      <div class="field" style="margin-top:14px">
        <label>Organizare</label>
        <div class="toggle-group">
          <button class="toggle-btn active" data-val="ext" onclick="setModOrganizare(this)">Extensie / An / Lună</button>
          <button class="toggle-btn" data-val="structura" onclick="setModOrganizare(this)">Păstrează structura foldere</button>
        </div>
      </div>
    </div>
...
<script>
  ...
  function setModOrganizare(btn) {
    btn.parentElement.querySelectorAll('.toggle-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    modOrganizare = btn.dataset.val;
  }

  async function scaneazaExtensii() {
    const sursa = document.getElementById('sursa').value.trim();
    if (!sursa) { toast('Completează sursa înainte de scanare!', 'err'); return; }
    try {
      const r = await fetch('/scaneaza?sursa=' + encodeURIComponent(sursa), {
        headers: {'X-FileFlow-Token': TOKEN}
      });
      if (!r.ok) throw new Error(await r.text());
      const rezultat = await r.json();
      const cont = document.getElementById('ext-chips');
      if (!rezultat.length) { cont.innerHTML = '<span class="switch-sub">Niciun fișier găsit în sursă.</span>'; return; }
      cont.innerHTML = rezultat.map(it =>
        `<button type="button" class="filter-btn active" data-ext="${it.ext}" onclick="toggleExt(this)">${it.ext} (${it.nr})</button>`
      ).join('');
      syncExtInclude();
    } catch(e) {
      toast('Eroare scanare: ' + e.message, 'err');
    }
  }
  ...
  function syncExtInclude() {
    const chips = [...document.querySelectorAll('#ext-chips .filter-btn')];
    const active = chips.filter(c => c.classList.contains('active'));
    document.getElementById('ext_include').value =
      (active.length === chips.length) ? '' : active.map(c => c.dataset.ext).join(' ');
  }
  ...
  async function startOrganizare() {
    ...
    try {
      const r = await fetch('/start', {
        method: 'POST',
        headers: {'Content-Type': 'application/json', 'X-FileFlow-Token': TOKEN},
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            pagina = HTML.replace('__TOKEN__', TOKEN)
            ...
            if not secrets.compare_digest(self.headers.get('X-FileFlow-Token', ''), TOKEN):
                self._send(403, 'text/plain', b'Token invalid'); return
            sursa = parse_qs(urlparse(self.path).query).get('sursa', [''])[0]
            if not os.path.isdir(sursa):
                self._send(400, 'text/plain', f'Sursa nu exista: {sursa}'.encode()); return
            contor = Counter()
            for cale in colecteaza_fisiere(sursa):
                ext = os.path.splitext(cale)[1].lower().lstrip('.') or 'fara_extensie'
                contor[ext] += 1
            rezultat = sorted(
                ({"ext": e, "nr": n} for e, n in contor.items()),
                key=lambda x: -x["nr"]
            )
            self._send(200, 'application/json', json.dumps(rezultat).encode())
        else:
    ...
    def do_POST(self):
        if self.path == '/start':
            ...
            sursa = cfg.get("sursa", "")
            dest  = cfg.get("destinatie", "")
            if not os.path.isdir(sursa):
                self._send(400, 'text/plain', f'Sursa nu exista: {sursa}'.encode()); return
            if os.path.abspath(sursa) == os.path.abspath(dest):
                self._send(400, 'text/plain', b'Sursa si destinatia nu pot fi identice'); return
